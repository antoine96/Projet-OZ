...
<<<<<<< HEAD
   ...
   in
      case ZombieListe of nil then MapListe#R.1#L#Dir
      [] r(X Y)|T then
	 local Res in
	    {ChooseDirection N}
	    Res={ZombieGame X Y Command.N MapListe R L Dir}
	    if Res==1 then
	       1
	    else
...
<<<<<<< HEAD
		  ...
		  elseif IX==Xporte andthen IY==Yporte then  if Nobjettake>=NObjetNeeded then {UserCommand win|nil Count IX IY List Nammo Nobjettake R L Dir}
							     else
								{DrawBox Floor X Y}
								{NiceBrave DX DY IX IY}
